...
assert.equal(vm.runInContext('WEBUI_VERSION', context), '1.2.0');
...
assert(!vm.runInContext('responseMatcher("SAVE F 8")("SENSOR 0 800 800 1 7 99 7.4")',context));
assert(!vm.runInContext('responseMatcher("SAVE F 8")("OK SAVED B=8")',context));
vm.runInContext('var resolved = false; pendingResponse = {matches:responseMatcher("SAVE DEF 2"),resolve:()=>resolved=true,reject:()=>{},timeout:0};handleSerialLine("SENSOR 0 800 800 1 5 99 7.4")',context);
